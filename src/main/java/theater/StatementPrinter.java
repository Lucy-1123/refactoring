...
public class StatementPrinter {
    ...
    private int getAmount(Performance performance) {
        int amount = 0;
        final Play p = getPlay(performance);
        switch (p.getType()) {
            case "tragedy":
                amount = Constants.TRAGEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    final int b = Constants.TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON;
            ...
            case "comedy":
                ...
                break;
            default:
                throw new RuntimeException(String.format("unknown type: %s", p.getType()));
        }
        return amount;
    }
}
